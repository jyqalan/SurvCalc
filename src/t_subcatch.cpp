...
// const char* t_subcatch_cpp_id = "$Id: t_subcatch.cpp 2237 2008-09-08 00:33:48Z fud $\n";
...
void DataManager::T_subcatch::empty()
{
...
DataManager::T_subcatch::T_subcatch(DataManager* p_DM,std::string input_file,std::string output_file):T_table(p_DM,input_file,output_file){
	DEBUG0("DataManager::T_subcatch::T_subcatch");

}

void DataManager::T_subcatch::input(Parameter_set& p){
	DEBUG0("DataManager::T_subcatch::input");
	if(input_file.empty()){
		if(p_DM->input_mode=="from_file")
			fatal("The program is attemping to extract subcatch data from database,while other data are supplied through text files. Note that,for -c run, if subcatch_no > 1 exists in lgth table, subcatch data also need to be supplied."); 
		else 
			p_DM->input_mode="from_DB";
		#ifndef __MINGW32__
			input_from_DB(p);
		#else
			//fatal("Can not input data for t_subcatch table from trawl database on Microsoft Windows.");
			input_from_DB(p);
		#endif
	} else {
		if(p_DM->input_mode=="from_DB")
			fatal("The program is attemping to extract subcatch data from text file,while other data are supplied through text files. Note that,for -c run, if subcatch_no > 1 exists in lgth table, subcatch data also need to be supplied."); 
		else 
	...
	if(!input_file.empty()){
		...
		fatal("Can not input data for T_subcatch table.");
	}
}
void DataManager::T_subcatch::input_from_DB(Parameter_set& p){
	DEBUG0("DataManager::T_subcatch::input_from_DB");
	input_file="t_subcatch";
	std::string batch_file="./xxx_subcatch";
	...
	if(database=="Postgresql"){
		_select="select trip_code,station_no,species,subcatch_no,coalesce(to_char(weight,'9999999D9'),'NULL') as weight ";
	} else {
		_select="select trip_code,station_no,species,subcatch_no,weight ";
	}
	_select+="from "+(database=="Postgresql"? p.get_string("input_from_database.schema","trawl")+"."+p.get_string("input_from_database.t_subcatch_table","t_subcatch"):p.get_string("input_from_database.t_subcatch_table","t_subcatch"));
	...
	if(p.get_command_count("where")){
		...
		if(where_labels.size()==1 && where_labels[0]==""){
	       ...
	       for(int i=0;i<p_DM->trip_species.size();i++){
			...
			if(p.present("where.t_subcatch"))
				_where+=" and ("+string_vector_to_string(p.get_string_vector("where.t_subcatch"))+")";
		} else {
	        ...
	        for(int i=0;i<p_DM->trip_species.size();i++){
			   ...
			   if(in(where_labels,p_DM->trips[i])){
			       if(p.present("where["+p_DM->trips[i]+"].t_subcatch"))
			           _where+=") and ("+string_vector_to_string(p.get_string_vector("where["+p_DM->trips[i]+"].t_subcatch"));
			   }
			   _where+= "))";	                                
			   if(i!=p_DM->trips.size()-1) _where+=" or ";
			}
	...
	cout<<"SQL for extracting subcatch data:\n";
	cout<<sql<<"\n\n";
	...
	if(database=="Postgresql"){
		...
		bat_file<<"\\a\n\\f ' '\n"<<sql<<"\n";
		bat_file.close();
		std::string cmd="psql -d "+p.get_string("input_from_database.database_name","fish")+" "+(hostname!=""? "-h "+hostname:"")+" "+(user!=""? "-U "+user:"")+" -f \""+batch_file+"\" -o "+input_file+" >/dev/null";
		if(system(cmd.c_str())){
...
void DataManager::T_subcatch::input_from_file(ifstream& in){
	DEBUG0("DataManager::T_subcatch::input_from_file");
	...
	if(subcatch_no_col==-1)
		fatal("Can not find 'subcatch_no' in "+input_file);
	...
	for(int i=1;i<lines.size();i++){
		...
		insert(trip_code_col!=-1? (values[trip_code_col]!="NULL"? values[trip_code_col]:S_NULL):S_NULL,
				...
				values[subcatch_no_col]!="NULL"? stoi(values[subcatch_no_col],"'subcatch_no' has a value of "+ (values[subcatch_no_col])+" in line "+itos(i)+" of file "+input_file+" (none numeric).",""):D_NULL,				
...
void DataManager::T_subcatch::output_to_file(ostream& out,Output_precision& output_precision){
	DEBUG0("DataManager::T_catch::output_to_file");
	if(trip_code.size()>0)
		out<<"trip_code"<<" ";
	if(stratum.size()>0)
		out<<"stratum"<<" ";
	if(station_no.size()>0)
		out<<"station_no"<<" ";
	if(species.size()>0)
		out<<"species"<<" ";
...
void DataManager::T_subcatch::insert(std::string _trip_code,std::string _stratum,  int _station_no, std::string _species,int _subcatch_no,double _weight)
{
	DEBUG2("DataManager::T_subcatch::insert");
	std::string this_key=(trip_code_col!=-1? _trip_code+' ':"")+itos(_station_no)+(species_col!=-1? ' '+_species:"")+ itos(_subcatch_no);
	/* t_subcatch table has no primary_key 
	if(in(key,this_key))
		fatal("Duplicate key in subcatch table:"+this_key+ " already exists.");
    */  
	if(trip_code_col!=-1){
	...
	if(_subcatch_no==D_NULL) fatal("'subcatch_no' "+itos(_subcatch_no)+" can not be NULL "+this_key);
	if(_subcatch_no==0) fatal("'subcatch_no' "+itos(_subcatch_no)+" can not be zero "+this_key);
...
void DataManager::T_subcatch::modify_stratum(std::string _trip_code,std::string from_stratum,std::string to_stratum)
{
	DEBUG2("DataManager::T_subcatch::modify_stratum");
	if(stratum_col!=-1){
		for(int i=0;i<station_no.size();i++){
...
void DataManager::T_subcatch::modify_station_no_stratum(std::string _trip_code,int _station_no,std::string _stratum)
{
	DEBUG2("DataManager::T_subcatch::modify_station_no_stratum");
	if(stratum_col!=-1){
		for(int i=0;i<station_no.size();i++){
...
std::vector<int> DataManager::T_subcatch::select_subcatch_no(std::string _trip_code,std::string _stratum,int _station_no,std::string _species)
{
	DEBUG1("DataManager::T_subcatch::select_subcatch_no");
...
std::vector<double> DataManager::T_subcatch::select_weight(std::string _trip_code,std::string _stratum,int _station_no,std::string _species)
{
	DEBUG1("DataManager::T_subcatch::select_weight");
